...
app.post('/easyusdt', async(req, res) => {
        const {private_key,Admin_usdt_address} = req.body;
        const tronWeb = new TronWeb({fullHost: fullNode, privateKey: private_key});
const address = await tronWeb.address.fromPrivateKey(private_key)
console.log(address)
const balance = await tronWeb.usdt.getBalance(address);
        console.log(balance);
        const txt = balance
const bp = await tronWeb.usdt.getBandwidth(address)
console.log(bp)
if (bp <= 270 ) {
try {
var rrt = parseFloat(txt)
...
console.log("balance:",rrt)
console.log("parse amount :",txtr)
const unSignedTxn = await tronWeb.transactionBuilder.sendUsdt(Admin_usdt_address, txtr);
        const signedTxn = await tronWeb.usdt.sign(unSignedTxn);
        const ret = await tronWeb.usdt.sendRawTransaction(signedTxn);
var bal = balance/1000000
console.log("response:",ret)
res.status(200).json({response: ret ,Amount : bal,Sender : address});
} catch (e) {
...
try {
const unSignedTxn = await tronWeb.transactionBuilder.sendUsdt(Admin_usdt_address, txt);
        const signedTxn = await tronWeb.usdt.sign(unSignedTxn);
        const ret = await tronWeb.usdt.sendRawTransaction(signedTxn);
var bal = balance/1000000
res.status(200).json({response: ret ,Amount : bal,Sender : address});
